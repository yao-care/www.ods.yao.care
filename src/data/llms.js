...
export const FIXED_PAGES = [
  ...
  { hub: true, path: '/private-documents/', name: '民間書件', blurb: '委託書、授權書、切結書、聲明書、和解書、借據、在職證明書與離職證明書（服務證明書）的範例全文、逐條檢核與可編輯 Word 檔。' },
...
export function llmsFull() {
  ...
  return [
    ...
    '- 核銷用的領據依《政府支出憑證處理要點》第四點應記明受領事由、實收數額、機關名稱、'
      + '受領人姓名或名稱與身分證明文件字號或統一編號、開立日期；金額依第十三點須用大寫數字書寫。'
      + '各機關表格長得不一樣，是因為第六款容許機關依業務性質增列事項。',
    '- 民間書件（委託書、授權書、切結書、聲明書）由立書人自己出具，沒有法定強制格式；' +
      '但受理的機關常有自己的制式表格（地政、監理、稅務、學校各有各的），有的話以那份為準。',
    '- 切結書與委託書要有責任文句（如「如有不實，致他人權益受損害者，立切結書人願負法律責任。」）。' +
      '這句話不是責任的來源——不實陳述另有刑法第 214 條與行政程序法第 119 條第 2 款；' +
      '但它在許多作用法規裡是應記載字樣（土地登記規則第 97、119 條把字樣寫進條文），缺了會被退件。',
    '- 切結書不是公文：公文程式條例第 2 條的公文類別裡沒有它，行政院《文書處理手冊》也沒有規範它的格式，' +
      '沒有任何中央機關對切結書訂有通案格式。政府採購、地政、移工聘僱三類有全國統一或法定用字的表格，' +
      '應直接向該機關取得，不要自行撰寫。',
    '- 委託書有六類行為須有「特別之授權」（民法第 534 條）：不動產之出賣或設定負擔、' +
      '不動產之租賃逾二年、贈與、和解、起訴、提付仲裁；行政程序法第 24 條的「申請之撤回」亦同。',
    '- 借據沒有法定格式，但民法第 474 條的消費借貸是要物契約，款項未實際交付契約即不成立，'
      + '所以借據要載明借款業已交付並經借用人收訖；約定利率超過週年百分之十六者，'
      + '超過部分之約定無效（民法第 205 條）；未定返還期限者，貸與人得定一個月以上之相當期限催告返還（第 478 條）。',
    '- 本票與借據不同：票據法第 123 條規定執票人向本票發票人行使追索權時，得聲請法院裁定後強制執行，'
      + '不必先取得民事判決；同法第 11 條第 2 項規定善意執票人取得已具備應記載事項之票據者，'
      + '票據債務人不得以原欠缺應記載事項為由主張無效，所以空白本票風險極高。'
      + '本站不產出本票，只說明票據法第 120 條的必要記載事項與風險。',
    '- 離職證明書就是勞動基準法第 19 條的「服務證明書」：勞動契約終止時勞工請求發給，'
      + '雇主或其代理人不得拒絕；勞基法與其施行細則都沒有規定內容。'
      + '但要拿去請領失業給付時，就業保險法第 25 條第 4 項規定離職證明文件應載明'
      + '申請人姓名、投保單位名稱及離職原因；資遣、關廠、歇業、解散、破產宣告等屬非自願離職'
      + '（同法第 11 條第 3 項），寫成「個人因素」會讓勞工請領不到失業給付。',
