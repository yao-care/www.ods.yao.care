...
⚠️ **判讀邊界**：這是 Bing 的量，只能看相對大小與「是不是 0」。有量 ≠ 打得贏，第 2 步不能省。

組合字的 0 該不該信，**2026-08-27 量清楚了**（原本一律寫「多字組合的 0 不可靠」，那太寬）：

- **中文＋英數（word／excel／pdf／英文詞）的組合：API 一律回 0，完全不可信。**
  鐵證：`word` 本身 exact 63,864、`履歷表` 2,628，但「履歷表 word」回 0 ——
  那是台灣最常搜的組合之一，不可能真的是 0。「切結書 word」「委託書 word」「報價單 word」同樣全 0。
- **中文＋中文的組合：API 切得出來，主詞有量而組合是 0，那個 0 就是實訊。**
  切結書範例 399、委託書範例 763、借據範本 858（比主詞借據 435 還大）、收據範本 647、
  和解書範例 341、履歷範本 831、報價單範本 550 —— 都量得到。
- **主詞本身就 0 的**，組合的 0 當然也是實訊（補助核銷 0、公文速別 0、核稿 0、公文大腦 0）。
- `契約範本` broad 305／exact 0 —— **broad 與 exact 會分歧，兩欄都要看。**

由此得到兩條命名結論（見 `docs/keyword-validation/2026-08-27-combo-zero-resolved.md`）：
**「存證信函怎麼寫」是實訊 0**（主詞 2,058），問句型在這個領域沒有量，
站上「怎麼寫」系列靠的是主詞，**不要再為問句型另開頁**；
**「公文範本」0 而「公文格式」300**，市場的詞是「格式」，`/templates/` 標題裡那兩個字不能拿掉。
民間書件則相反 —— 「範本／範例」後綴是主要搜法。
...
- `liability_clause`（critical）：切結書與委託書要有「如有不實…願負法律責任」的文句。
  **這句話的作用要講準**（2026-08-26 查證後修正過一次）：不實陳述的責任不是這句話創設的
  —— 那來自刑法第 214 條與行政程序法第 119 條第 2 款；沒寫並不代表文件無效。
  但它在許多作用法規裡是**法定應記載字樣**（土地登記規則第 97 條把「如有不實，出賣人願負法律責任」
  直接寫進條文，第 119 條第 4 項的繼承系統表同理），缺了會被退件。
  **這條擋的是「會被退件」，不是「法律上不成立」——站上文案不要再寫成後者。**
...
但段名檢核不認的話會當場被擋下（借據實測踩過）。

**借據也在這個族群，但本票刻意不做**（2026-08-26）：借據沒有法定格式，它是消費借貸的證明，
民法第 474 條的消費借貸是**要物契約**——錢沒有實際交付契約就不成立，
所以有 `loan_delivery`（critical）要求載明交付；`interest_cap`（warn）對應民法第 205 條的
週年 16% 上限。**本票不產出**：那是有價證券，票據法第 123 條讓執票人可以聲請法院裁定後
...
唯一有法定必載清單的是拿去請領失業給付的用途：就業保險法第 25 條第 4 項
...
- `settlement_scope`（critical）：和解書必須有「拋棄範圍與保留事項」段，而且不能只寫
  「拋棄其餘一切請求」——要出現「僅就／限於／不包括／保留／除…外」這類界定字樣。
  依據：民法第 737 條（和解使當事人所拋棄之權利消滅）＋兩則實務對照 ——
  **臺灣新竹地方法院 110 年度簡上字第 111 號**：一份 3,000 元的和解書因為
  「並無其他損害賠償請求權保留之陳述或記載」，關掉了事後 1,253 萬元的請求；
  反面的**臺灣高等法院 108 年度上字第 904 號**則是因為和解書載明「僅就系爭機車損害」，
  人身損害才不在拋棄射程內。差別就在這一段有沒有寫、寫了什麼。
- `settlement_insurance`（warn）：內容涉及車禍時，應載明強制汽車責任保險給付是否含在和解金內。
  強制汽車責任保險法第 7 條、第 34 條第 1 項第 2 款：受害人可直接以書面通知保險人請求，
  不需加害人同意也不需先和解 —— 先簽和解等於把本來拿得到的錢當籌碼送掉。

`/private-documents/settlement-*` 兩頁上方有紅框警語（判決、強制險、保險人參與、
金保法第 24 條第 2 項的評議中心、刑法第 276 條過失致死非告訴乃論），
並勸使用者走鄉鎮市區調解委員會。**這幾段不要為了版面精簡刪掉。**

⚠️ **引用裁判前先驗當事人，不要只驗案號**（2026-08-27 的教訓，台灣端查出來的）：
**地方法院案號會跨院重複。** 用「105年度訴字第1513號」全文檢索確實中一筆臺灣高等法院判決，
但那是**士林地院**同案號的上訴審，案由「返還共有物」、當事人一個都對不上 ——
**逐字核對擋不住這種錯**，抄回來的每個字都是真的，錯在它不是這件事。
另一條：**「FJUD 查無」≠「沒有上訴審」**（可能未繳費被駁回、撤回上訴、和解、或裁判未公開），
查無就寫查無。**一審判決而上訴結果不明的，不要引用** —— 二審若翻掉，站上就是在教錯的事。
本主機取不到 FJUD 全文（查詢端點 `Connection reset`），要查走 `docs/TODO-FOR-TAIWAN.md` 的台灣端工單。
...
**切結書不是公文**：公文程式條例第 2 條的公文類別（令、呈、咨、函、公告、其他公文）裡沒有它，
行政院《文書處理手冊》全文也沒有「切結」二字 —— **沒有任何中央機關對切結書訂有通案格式**。
真正在管內容的是個別作用法規。由此得出這一區該不該下場的判準：
**該情境沒有機關表格 → 可以生成；有機關表格 → 那份表格的用字往往是法規逐字規定的，
自己生成等於讓案件被駁回。** 政府採購（工程會全國統一版）、地政（切結文字寫進條文）、
移工聘僱（勞動部五語官方版、聘僱許可必備）這三類一律指向機關，不要自己寫。
...
6. ~~民間書件（委託書、授權書、切結書、聲明書、和解書）~~：**已全部上線**
   ...
   **領據**（`/receipt/`）與**職場證明**（在職證明、離職證明書）都已做。
