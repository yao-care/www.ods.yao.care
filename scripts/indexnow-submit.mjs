...
// 補尾斜線：模式 B（project pages）的 SITE_URL 含 base 子路徑，少了尾斜線會被 new URL 當檔名解析掉，
// sitemap/keyLocation 全跑到主機根目錄（2026-07-28 ttpa 實測）。
...
const payload = {
  host: site.host,
  key: KEY,
  // 相對解析（不是 `/${KEY}.txt`）：模式 B 的金鑰檔在 base 子路徑下，寫死主機根目錄會 404 → IndexNow 403。
  // 送出的 URL 都在該子路徑之下，符合 IndexNow「keyLocation 需與提交 URL 同目錄或其上層」的規定。
